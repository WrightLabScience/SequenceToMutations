...
strains <- gsub('ancestor_([0-9]+)_out', '\\1', grepv('ancestor', lf))
...
for (s in seq_along(strains)) {
   ...
   lineages <- gsub('^evolved_([0-9]{1,2}[a-z]_[A-Z]{3}(m|p))_out$', '\\1', evolved_fnames)
   ...
   for (l in seq_along(evolved_fnames)) {
      ...
      # If any ancestor mutations were found in this evolved lineage, remove it.
      if (length(trow) > 0) 
         evolved_muts <- evolved_muts[-trow, ]
      
      num_muts <- c(num_muts, nrow(evolved_muts))
      
      # Write subtracted ancestor mutations list to .txt
